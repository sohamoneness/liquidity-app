...
let CartPage = class CartPage {
    constructor(authService, navCtrl, userDetails, helper, datePipe, platform) {
    ...
    DeceasePriceValueforTheOrderExceptThisIds(ExceptIds) {
        ...
        this.userDetails.getLiquorDataExceptTheseIds(ExceptIds).subscribe(res => {
            this.newData = res;
    ...
    filterDataAndUpdate(ExceptedID, newData) {
        ...
        newData.forEach((value) => {
            if (ExceptedID.find(x => x == value.id) == undefined) {
                let nowPrice = parseFloat(value.BigLiquorActualPrice) - (parseFloat(value.BigLiquorActualPrice) * parseFloat(this.priceDecreaseByPercentage));
