        ...
        _createClass(WalletPage, [{
          ...
          value: function ngOnInit() {
            ...
            if (localStorage.getItem("userDetails")) {
              ...
              this.accountProvider.fetchWalletTransactions(id).then(function (res) {
                if (res["status"] == "1") {
                  _this.walletList = res["wallets"];
                  _this.wallet_balance = res["wallet_balance"];
                } else {
          ...
          value: function pay() {
            ...
            var addToWallet = function addToWallet(payment_id) {
              var data = _this2.loginForm.value;
              data.user_id = JSON.parse(localStorage.getItem("userDetails")).id;

              _this2.accountProvider.addWalletBalance(data).then(function (res) {
                if (res["status"] == "1") {
                  _this2.helper.presentAlert('Amount successfully added to your wallet.');

                  if (localStorage.getItem("userDetails")) {
                    var id = JSON.parse(localStorage.getItem("userDetails")).id;

                    _this2.accountProvider.fetchWalletTransactions(id).then(function (res) {
                      console.log("rohan", res);

                      if (res["status"] == "1") {
                        _this2.walletList = res["wallets"];
                        ...
                        _this2.accountProvider.fetchWalletTransactions(_id).then(function (res) {
                          // console.log("rohan", res)
                          if (res["status"] == "1") {
                            _this2.walletList = res["wallets"];
                            _this2.wallet_balance = res["wallet_balance"];
                            _this2.wallet_amount = '';

                            _this2.helper.suucessAlert(res["message"]);

                            _this2.navCtrl.navigateRoot('/homenew');
                          } else {
